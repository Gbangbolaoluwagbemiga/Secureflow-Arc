...
/**
 * Deploys SecureFlow behind an ERC1967 proxy.
 *
...
contract DeployScript is Script {
    function run() external {
        ...
        SecureFlow implementation = new SecureFlow();
        ...
        ERC1967Proxy proxy = new ERC1967Proxy(
            ...
            abi.encodeCall(SecureFlow.initialize, (feeCollector, platformFeeBP))
        );

        console.log("SecureFlow implementation:", address(implementation));
        console.log("SecureFlow PROXY (use this one):", address(proxy));
        console.log("version:", SecureFlow(payable(address(proxy))).version());
