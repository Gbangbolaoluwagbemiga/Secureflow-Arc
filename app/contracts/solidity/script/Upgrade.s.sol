 ...
 *      cannot undo.
 *   2. Confirm the storage rules at the top of SecureFlow.sol were followed:
...
contract UpgradeScript is Script {
    function run() external {
        ...
        SecureFlow proxy = SecureFlow(payable(proxyAddress));
        string memory before = proxy.version();
        uint256 escrowsBefore = proxy.nextEscrowId();

        vm.startBroadcast(deployerPrivateKey);

        SecureFlow newImplementation = new SecureFlow();
