...
import "../src/yield/SecureFlowYield.sol";
...
contract DeployYieldTestnetScript is Script {
    ...
    function run() external {
         ...
         *
         * SecureFlowYield is not upgradeable, so replacing it means pointing the
         ...
         */
        address current = address(SecureFlow(proxy).yieldController());
        if (current != address(0)) {
            address heldToken = token;
            uint256 stillOut = SecureFlowYield(payable(current)).deployedAssets(heldToken);
            require(stillOut == 0, "old controller still has capital deployed; unwind it first");
        }

        vm.startBroadcast(pk);

        SecureFlowYield controller = new SecureFlowYield(proxy);
        SecureFlow(proxy).setYieldController(address(controller));
        ...
        console.log("SecureFlowYield:     ", address(controller));
