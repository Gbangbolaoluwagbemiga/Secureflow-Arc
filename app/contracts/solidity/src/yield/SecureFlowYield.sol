...
/** The slice of SecureFlow this controller needs to reason about safety. */
interface ISecureFlowEscrows {
 ...
 * smaller and worse. Two attempts at shaving bytes are recorded in SecureFlow.sol
 ...
 */
contract SecureFlowYield is ISecureFlowYield, Ownable2Step, ReentrancyGuard {
    ...
    function setYieldOptIn(uint256 escrowId, bool optedIn) external {
        ISecureFlowEscrows.Escrow memory esc = ISecureFlowEscrows(escrow).getEscrow(escrowId);
        ...
        if (esc.workStarted || esc.status != ISecureFlowEscrows.EscrowStatus.Pending) {
    ...
    function investableCeiling(uint256 escrowId) public view returns (uint256) {
        ...
        if (
            esc.status != ISecureFlowEscrows.EscrowStatus.Pending &&
            esc.status != ISecureFlowEscrows.EscrowStatus.InProgress
        ) return 0;
        ...
        ISecureFlowEscrows.Milestone[] memory ms = ISecureFlowEscrows(escrow).getMilestones(escrowId);
        for (uint256 i; i < ms.length; ++i) {
            if (ms[i].status == ISecureFlowEscrows.MilestoneStatus.Approved) continue;
    ...
    function investableAmount(uint256 escrowId) public view returns (uint256) {
        ...
        address token = ISecureFlowEscrows(escrow).getEscrow(escrowId).token;
    ...
    function investIdle(uint256 escrowId) external nonReentrant {
        ...
        ISecureFlowPull(escrow).releaseToYield(token, amount);
    ...
    function distributeYield(uint256 escrowId) external nonReentrant {
        ...
        address platform = ISecureFlowEscrows(escrow).feeCollector();
