...
import "./yield/ISecureFlowYield.sol";
 ...
 * SecureFlowYield, reached through a single controller pointer, and the split is
    ...
    UUPSUpgradeable
{
    ...
    /* Retained for storage layout only. Nothing reads it any more, so the
       public getter is gone; the slot must stay exactly where it is or the
       next upgrade reinterprets every mapping declared after it. */
    mapping(uint256 => mapping(address => bool)) internal disputeVotes;
     ...
     * existed.
     *
     * (It is also what brought SecureFlow back under EIP-170. Combined, the two
     ...
     */
    ISecureFlowYield public yieldController;
    ...
    /**
     * @dev May `who` rule on this escrow? Caller has already passed onlyArbiter.
     *
     *      The panel a client named decides, but only while somebody on it can
     *      actually act. createEscrow does not require named arbiters to hold
     *      protocol authority, and authority is revocable besides — so a panel
     *      can be composed of addresses that never could arbitrate, or be
     *      emptied by revokeArbiter while a dispute is open. Enforcing the
     *      panel unconditionally would shut both doors at once and leave the
     *      escrow's money with nowhere to go.
     *
     *      So: no panel, the protocol's arbiters govern. A panel with anyone
     *      live on it, only that panel rules. A panel with nobody live, the
     *      protocol's arbiters are the backstop. A job never becomes
     *      unarbitrable.
     *
     *      One pass answers both questions, because the loop that looks for the
     *      caller is the same loop that would look for a live member.
     */
    function _panelGate(Escrow storage esc, address who) internal view returns (bool) {
        uint256 n = esc.arbiters.length;
        if (n == 0) return true;
        bool anyLive;
        for (uint256 i; i < n; ++i) {
            address a = esc.arbiters[i];
            if (a == who) return true;
            if (authorizedArbiters[a]) anyLive = true;
        }
        return !anyLive;
    }
    ...
    function createEscrow(
        ...
        /*
         * The fee is held, not earned.
         *
         * Its tokens arrive with the deposit and stay in the contract, but they
         * are not counted as revenue until the job settles, because until then
         * cancelJob, setMilestones and withdrawJobFunds can all hand part of it
         * back. Counting it here is what let withdrawFees zero a balance that
         * clients still had a claim on, stranding their principal in jobs they
         * could no longer close. esc.platformFee is the record until then.
         */
        ...
        external nonReentrant whenNotPaused
    {
        ...
        if (esc.paidAmount == esc.totalAmount) {
            ...
            totalFeesByToken[esc.token] += esc.platformFee; // earned now
    ...
    function resolveDispute(
        ...
        // job has been through arbitration". That is the only thing still
        // asking, and it asks a yes/no question, so a marker is enough — the
        // old per-arbiter dedup existed to keep an exact tally that nothing
        // reads any more.
        if (disputeVoteCounts[escrowId] == 0) disputeVoteCounts[escrowId] = 1;

        // Quorum is per milestone AND per proposed split, so it measures
        // agreement on an outcome rather than attendance.
        bytes32 proposal = keccak256(abi.encode(freelancerAmount, clientAmount));
        if (!milestoneDisputeVotes[escrowId][milestoneIndex][proposal][msg.sender]) {
            milestoneDisputeVotes[escrowId][milestoneIndex][proposal][msg.sender] = true;
            milestoneDisputeVoteCounts[escrowId][milestoneIndex][proposal]++;
        }

        uint256 agreeing = milestoneDisputeVoteCounts[escrowId][milestoneIndex][proposal];
        emit DisputeVoteCast(escrowId, msg.sender, agreeing);

        if (agreeing < esc.requiredConfirmations) return;
    ...
    function emergencyRefundAfterDeadline(uint256 escrowId) external nonReentrant {
        ...
        /*
         * An open dispute outranks the clock.
         *
         * This is the escape hatch for a job that stalled, not a way to win an
         * argument by waiting. Without this check a client could refuse to
         * approve delivered work, let the freelancer raise a dispute, sit out
         * the thirty days while the arbiter deliberated, and then reclaim the
         * whole unpaid balance including the milestone the freelancer had
         * already delivered. Outlasting arbitration would beat arbitration.
         *
         * Submitted work is held to the same rule: it is waiting on a decision
         * that has not been made, and the deadline passing is not that decision.
         */
        // disputeMilestone sets esc.status, so an open dispute is caught by the
        // status check alone; the scan only has to find work left delivered and
        // unreviewed, which leaves the escrow InProgress.
        if (esc.status == EscrowStatus.Disputed) revert CannotRefund();
        Milestone[] storage msRefund = escrowMilestones[escrowId];
        uint256 nRefund = msRefund.length;
        for (uint256 i; i < nRefund; ++i) {
            if (msRefund[i].status == MilestoneStatus.Submitted) revert CannotRefund();
        }
        ...
        totalFeesByToken[esc.token] += esc.platformFee; // kept, as before
    ...
    function setYieldController(address controller) external onlyOwner {
        yieldController = ISecureFlowYield(controller);
    }
    ...
    function setMilestones(
        ...
        if (newTotal > oldTotal) {
            ...
            // Paid out at the end of the function, not here. The milestone
            // array and esc.totalAmount below are still the old ones at this
            // point, and handing control to the depositor mid-rewrite lets a
            // contract depositor observe — and act on — a job whose money has
            // already left but whose terms have not yet been updated.
            refundOut = back + feeBack;
        } else {
            if (msg.value != 0) revert InvalidAmount();
        }
        ...
        if (refundOut > 0) _doTransfer(esc.token, address(this), esc.depositor, refundOut);
    ...
    function deleteEscrow(uint256 escrowId) external onlyOwner nonReentrant {
        ...
        /*
         * Read the parties out before the delete, not after.
         *
         * `esc` is a storage pointer: once escrows[escrowId] is deleted every
         * field it exposes reads as zero. Scrubbing the indexes afterwards
         * therefore cleaned address(0)'s array instead of the depositor's, and
         * the beneficiary branch could never run because the address it tests
         * had just been zeroed. Both parties were left holding an id in
         * getUserEscrows that resolves to a zeroed phantom escrow.
         */
        address depositorAddr = esc.depositor;
        address beneficiaryAddr = esc.beneficiary;

        delete escrows[escrowId];

        _removeFromUserEscrows(depositorAddr, escrowId);
        if (beneficiaryAddr != address(0)) {
            _removeFromUserEscrows(beneficiaryAddr, escrowId);
        }
     ...
     */
    /*
     * Dispute quorum, keyed per milestone rather than per escrow.
     *
     * disputeVotes/disputeVoteCounts above are keyed by escrow alone and are
     * never cleared, so once one dispute on an escrow reached quorum the
     * counter stayed there and every later dispute on that escrow executed on
     * the first arbiter's call. A client who asked for two confirmations kept
     * getting one, with nothing to tell them it had changed.
     *
     * Those two are deliberately left in place and still written to, because
     * `disputeVoteCounts[escrowId] > 0` is what reopenJob and withdrawJobFunds
     * read to mean "this job has been through arbitration". They are simply no
     * longer what gates a resolution.
     *
     * Votes are keyed by the split they endorse — keccak(freelancerAmount,
     * clientAmount) — so quorum means N arbiters agreeing on one outcome rather
     * than N arbiters having turned up. Before this, two arbiters proposing
     * opposite splits reached quorum between them and the money moved on
     * whichever of them called last.
     *
     * Keying by proposal also means nothing ever needs clearing: an arbiter who
     * changes their mind simply votes on a different split, and each split
     * accumulates its own tally independently.
     */
    mapping(uint256 => mapping(uint256 => mapping(bytes32 => mapping(address => bool))))
        public milestoneDisputeVotes;
    mapping(uint256 => mapping(uint256 => mapping(bytes32 => uint256)))
        public milestoneDisputeVoteCounts;

    /* 50 - 1 for the yieldController pointer above, - 2 for the per-milestone
       dispute mappings. Get this wrong and the next upgrade reinterprets live
       escrow data as whatever the new layout says; there is no revert, only
       wrong numbers. */
