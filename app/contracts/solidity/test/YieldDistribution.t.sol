...
contract YieldDistributionTest is JobManagerBase {
    SecureFlowYield internal yield_;
    MockYieldAdapter internal venue;

    function setUp() public override {
        super.setUp();

        yield_ = new SecureFlowYield(address(sf));
    ...
    function test_settlesOnlyOnce() public {
        ...
        yield_.distributeYield(id);
        vm.expectRevert(SecureFlowYield.AlreadySettled.selector);
        yield_.distributeYield(id);
    }
    ...
    function test_refusesWhileTheJobCouldStillMove() public {
        ...
        vm.expectRevert(SecureFlowYield.JobNotFinished.selector);
        yield_.distributeYield(id);
    }
    ...
    function _finish(uint256 id) internal {
        ...
        if (esc.status == SecureFlow.EscrowStatus.Released) return;

        SecureFlow.Milestone[] memory ms = sf.getMilestones(id);
        for (uint256 i; i < ms.length; ++i) {
            if (ms[i].status == SecureFlow.MilestoneStatus.NotStarted && ms[i].amount > 0) {
                _submit(id, i);
                vm.prank(client);
