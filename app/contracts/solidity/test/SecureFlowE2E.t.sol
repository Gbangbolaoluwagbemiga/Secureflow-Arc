 ...
 *   - SecureFlow's three client modes all end at the same contract state
 *
...
contract SecureFlowE2ETest is JobManagerBase {
    ...
    /**
     * Row one of SecureFlow's model: a human client who manages the job themselves.
    ...
    function test_e2e_manualJob_postToPaidAndRated() public {
        ...
        // Held, not earned. The fee's tokens are in the contract — the client
        // was debited for them on line 65 — but they are not revenue until the
        // job settles, because until then the client can still cancel or shrink
        // and get them back.
        assertEq(sf.totalFeesByToken(address(usdc)), 0, "fee is not revenue at creation");
        assertEq(
            usdc.balanceOf(address(sf)), BUDGET + expectedFee, "contract holds budget and fee"
        );
        _assertSolvent();
        ...
        // Settled, so now it is revenue and the collector can take it.
        assertEq(sf.totalFeesByToken(address(usdc)), expectedFee, "fee earned on release");
    ...
    /**
     * SecureFlow's whole point is that these coexist in one marketplace. Run all
