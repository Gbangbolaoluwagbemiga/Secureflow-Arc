...
import { arcChain, config, rpcUrl } from "../src/config.js";
...
async function main() {
  ...
  const clientWallet = createWalletClient({ account: clientAccount, chain: arcChain, transport: http(rpcUrl) });
  ...
  const walletClient = createWalletClient({ account: freelancerAccount, chain: arcChain, transport: http(rpcUrl) });
