...
import { config, arcChain, arcNetwork, explorerAddress, rpcUrl, logRpcUrl } from "./config.js";
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/api/wallet") {
    try {
      ...
      try {
        const publicClient = createPublicClient({ chain: arcChain, transport: viemHttp(rpcUrl) });
        balance = formatEther(
  ...
  if (req.method === "POST" && url.pathname === "/api/treasury/deposit") {
    try {
      ...
      const pub = createPublicClient({ chain: arcChain, transport: viemHttp(rpcUrl) });
