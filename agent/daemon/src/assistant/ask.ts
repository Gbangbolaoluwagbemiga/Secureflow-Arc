/**
 * Answering a question about SecureFlow.
 *
...
import { ASSISTANT_RULES, SECUREFLOW_KNOWLEDGE } from "./knowledge.js";
...
function describeViewer(v: Viewer | undefined): string {
  ...
  const bits: string[] = [];
  if (v.role === "client") bits.push("They hire on SecureFlow.");
  else if (v.role === "freelancer") bits.push("They take work on SecureFlow.");
  else if (v.role === "both") bits.push("They both hire and take work on SecureFlow.");
...
export async function askSecureFlow(
  turns: ChatTurn[],
  viewer?: Viewer,
): Promise<string> {
  const cleaned = turns
  ...
  const system = [
    ASSISTANT_RULES,
    "",
    "--- WHAT YOU KNOW ABOUT SECUREFLOW ---",
    SECUREFLOW_KNOWLEDGE,
    "",
